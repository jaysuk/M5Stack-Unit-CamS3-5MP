 ...
 *
 * Pulled from upstream espressif/esp32-camera (sensors/ov3660.c) and adapted for this
 * fork: renamed detect/init entry points to match this tree's sensor_func_t table
 * convention (ov3660_detect/ov3660_init, no esp32_camera_ prefix), dropped the af_*
 * autofocus fields (OV3660 has no AF and this fork's sensor_t has no af_* members at
 * all), and dropped set_xclk (this fork has no xclk_timer_conf/LEDC implementation —
 * ESP32-S3's LCD_CAM peripheral generates XCLK directly from camera_config_t.xclk_freq_hz,
 * see esp_camera.c/cam_hal.c; mega_ccm.c leaves set_xclk NULL for the same reason and it
 * is never called from the application layer).
 */
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
...
int ov3660_init(sensor_t *sensor)
{
    ...
    // Not implemented in this fork: XCLK is generated by the ESP32-S3 LCD_CAM
    // peripheral directly from camera_config_t.xclk_freq_hz (see cam_hal.c), not by
    // the sensor driver, so there is no xclk_timer_conf to call here.
    sensor->set_xclk = NULL;

    // PY260-vendor extensions on this fork's sensor_t; not applicable to OV3660 and
    // never called unconditionally by the application layer (main.c/http_server.c/
    // mqtt_mgr.c all NULL-check before calling sensor setters).
    sensor->set_Camera_rest = NULL;
    sensor->set_AGC_mode = NULL;
    sensor->set_mamual_exp_h = NULL;
    sensor->set_mamual_exp_l = NULL;

    ESP_LOGD(TAG, "OV3660 Attached");
    return 0;
}
